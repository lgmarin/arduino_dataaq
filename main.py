...
class BottomPanel(wx.Panel):
    def __init__(self, parent, top):
        ...
        com_ports = ["/dev/ttyUSB0", "/dev/COM3", "COM3", "COM5"]
        baud_rates = ["115200"]

        #Create TButton and create the toggle event (id = -1 - auto set)
        labelConnection = wx.StaticText(self, -1, label="Arduino Connection", pos=(10,10))
        ...
        self.togglebuttonStart = wx.ToggleButton(self, id=-1, label="Connect", pos=(10,95), size=(140,-1))
        self.togglebuttonStart.Bind(wx.EVT_TOGGLEBUTTON, self.OnStartClick)

        labelSaveCSV = wx.StaticText(self, -1, label="Record data to CSV", pos=(200,10))
        labelFileName = wx.StaticText(self, -1, label="Filename:", pos=(200,40))
        self.tbFileName = wx.TextCtrl(self, -1, "data", pos = (260,30), size=(150,-1))
        self.tbSaveCSV = wx.ToggleButton(self, id=-1, label="&Start Record", pos=(200,95), size=(120,-1))
        self.tbSaveCSV.Disable()
        self.tbSaveCSV.Bind(wx.EVT_TOGGLEBUTTON, self.OnRecordClick)

        labelGraphConfig = wx.StaticText(self, -1, label="Plot Configuration", pos=(450,10))
        ...
        self.buttonRange = wx.Button(self, id=-1, label="Set Axis", pos=(450,95), size=(120,-1))
        ...
        labelAbout3 = wx.StaticText(self, -1, label="Developper: Luiz Guilherme Marin", pos=(650,70))
        labelAbout4 = wx.StaticText(self, -1, label="Orientation: Dr. Ramón Cortés Paredes", pos=(650,90))
        labelAbout5 = wx.StaticText(self, -1, label="Co-Orientation: Dr. Christian Scapulatempo Strobel", pos=(650,105))
    ...
    def OnRecordClick(self, event):
        ...
        if (val == True):
            ...
            self.tbSaveCSV.SetLabel("Stop Record")
        else:
            print("Recording Stopped")
            self.tbFileName.Enable()
            self.recording = False
            self.dataFile.close()
            self.tbSaveCSV.SetLabel("Start Record")
    ...
    def OnStartClick(self, event):
        ...
        if (val == True):
            self.togglebuttonStart.SetLabel("Disconnect")
...
class Main(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, parent = None, title = "Arduino Data Acquisition", size = (1000,800), style=wx.SYSTEM_MENU | wx.CAPTION | wx.MINIMIZE_BOX | wx.CLOSE_BOX)
